...
bool duChkTmr(PTR cb, int16_t tmrEvnt)
{
   switch (tmrEvnt)
   {
      ...
      case EVENT_RIC_INDICATION_TMR:
      {
         if(((E2apDb *)cb)->e2Timers.ricIndicationTimer.tmrEvnt == EVENT_RIC_INDICATION_TMR)
         {
             DU_LOG("\nERROR  -->  DU_APP : duChkTmr: Invalid tmr Evnt [%d]", tmrEvnt);
             return TRUE;
         }
...
void duStartTmr(PTR cb, int16_t tmrEvnt, uint8_t timerValue)
{
   ...
   switch (tmrEvnt)
   {
      ...
      case EVENT_RIC_INDICATION_TMR:
      {
         ...
         arg.timers = &e2apDb->e2Timers.ricIndicationTimer;
         arg.max = MAX_RIC_INDICATION_TMR;
         break;
      }
      default:
      {
