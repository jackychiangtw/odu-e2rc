...
uint8_t smInit(){
    ...
    ranFuncInfo[INDEX_OF_KPM].fillRanFuncDescription = kpmFillRanFuncDescription;
    ranFuncInfo[INDEX_OF_KPM].procActionDefinition = kpmProcActionDefinition;
    ranFuncInfo[INDEX_OF_KPM].procEventTrigDefinition = kpmProcEventTrigDefinitionFormat1;
    ranFuncInfo[INDEX_OF_KPM].ranFunctionId = 2;
    ranFuncInfo[INDEX_OF_KPM].ranFunctionRev = 3;
    strcpy(ranFuncInfo[INDEX_OF_KPM].ranFunctionOId, oid_e2kpmv3);
...
uint8_t smProcEventTrigDefinition(RICeventTriggerDefinition_t *eventTrigDefini, uint32_t ranFuncId, long reqId, long instanceId){
    for(int i=0;i<SIZE_OF_SERVICE_MODEL;i++){
        if(ranFuncInfo[i].ranFunctionId == ranFuncId){
            return ranFuncInfo[i].procEventTrigDefinition(eventTrigDefini);
        }
    }
    DU_LOG("\nERROR  -->  E2SM Manager : Unknown RAN function ID");
    return RFAILED;
...
uint8_t smProcActionDefinition(RICactionDefinition_t *acitiondifin, uint32_t ranFuncId, long reqId, long instanceId){
    for(int i=0;i<SIZE_OF_SERVICE_MODEL;i++){
        if(ranFuncInfo[i].ranFunctionId == ranFuncId){
            return ranFuncInfo[i].procActionDefinition(acitiondifin);
        }
    }
    DU_LOG("\nERROR  -->  E2SM Manager : Unknown RAN function ID");
    return RFAILED;
