...
uint8_t kpmInit(){
    ...
    /*Initialize the KPM module parameters*/
    ...
    /*Initialize the cell and slice measurement name*/
...
uint8_t kpmFillRanFunctionName(RANfunction_Name_t *ranfunc_name){
    uint8_t ret = ROK;
    uint8_t ranfunc_defin[] = "ORAN-E2SM-KPM"; // RAN Function Definition,
    uint8_t ranfunc_descr[] = "KPM monitor"; // RAN function description
    uint8_t ranfunc_oid[] = "1.3.6.1.4.1.53148.1.3.2.2"; // RAN function service model OID of KPM
    ...
    ret = kpmFillOctetString(&ranfunc_name->ranFunction_E2SM_OID, ranfunc_oid);
    if(ret != ROK){
        DU_LOG("\nERROR  --> E2SM-KPM: Failed to assign RAN Function service model OID, %d",__LINE__);
        return RFAILED;
    }
...
uint8_t kpmFillTriggerStyleItem(RIC_EventTriggerStyle_Item_t *trigger_style){
    ...
    uint8_t ret = ROK;
    trigger_style->ric_EventTriggerStyle_Type = 1; // periodic report
...
uint8_t kpmFillTriggerStyleList(struct E2SM_KPM_RANfunction_Description__ric_EventTriggerStyle_List *triggerStyleList){
    ...
    ret = kpmFillTriggerStyleItem(trigger_style);
    if(ret != ROK){
...
uint8_t kpmFillReportStyleItemStyle1(RIC_ReportStyle_Item_t *report_style){
    ...
    uint8_t ret = ROK;
    report_style->ric_ReportStyle_Type = 1; // E2 Node Measurement
    ...
    report_style->ric_IndicationHeaderFormat_Type = 1; // defined in Table 7.8-1
    report_style->ric_IndicationMessageFormat_Type = 1;
    report_style->ric_ActionFormat_Type = 1; 
    ...
    /*Fill in the Measurement Info Action Item for cell metircs*/
    for(int i=0;i<SUPPORTED_CELL_MEAS_ITEM;i++){
...
/*******************************************************************
 *
 * @brief Fill in report style item in style 3
 *
 * @details
 *
 *    Function : kpmFillReportStyleItemStyle3
 *
...
uint8_t kpmFillReportStyleItemStyle3(RIC_ReportStyle_Item_t *report_style){
    uint8_t ret = ROK;
    ...
    report_style->ric_ReportStyle_Type = 3; // Condition-based, UE-level E2 Node Measurement
    report_style->ric_IndicationHeaderFormat_Type = 1; // defined in Table 7.8-1
    report_style->ric_IndicationMessageFormat_Type = 2;
    ...
    /*Fill in the Measurement Info Action Item for slice metircs*/
    for(int i=0;i<SUPPORTED_SLICE_MEAS_ITEM;i++){
...
uint8_t kpmFillReportStyleList(struct E2SM_KPM_RANfunction_Description__ric_ReportStyle_List *reportStyleList){
    ...
    /*Fill in the Report Style Item for cell metrics*/
    ret = kpmFillReportStyleItemStyle1(report_style);
    if(ret != ROK){
        DU_LOG("\nERROR   -->  E2SM-KPM : Failed to fill report style item style 1, %d", __LINE__);
        return RFAILED;
    }
    /*Fill in the Report Style Item for slice metrics*/
    ret = kpmFillReportStyleItemStyle3(report_style+1);
    if(ret != ROK){
...
uint8_t kpmFillRanFuncDescription(RANfunctionDefinition_t  *ranFunDefinition){
    ...
    ret = kpmFillRanFunctionName(&ranfunc_desc->ranFunction_Name);
    if(ret != ROK){
    ...
    /*Encode the RAN Function Description*/
...
uint8_t kpmProcEventTrigDefinitionFormat1(RICeventTriggerDefinition_t *defini){
    ...
    if(rval.code == RC_FAIL || rval.code == RC_WMORE)
   {
    ...
    /* duCb.duTimersInfo.tmrRes hasn't been initialized, skip it*/
    // if((duChkTmr((PTR)&(duCb.e2apDb), EVENT_RIC_INDICATION_TMR)) == FALSE) 
    // {
    //     duStartTmr((PTR)&(duCb.e2apDb), EVENT_RIC_INDICATION_TMR, reportingPeriod);
    // }
    // else
    // {
    //     DU_LOG("\nERROR   -->  E2AP : EVENT_RIC_INDICATION_TMR timer is already running");
    //     return;
    // }
 ...
 * @return index, int, -1 means not support
 *
 ******************************************************************/
...
/*******************************************************************
 *
 * @brief Process the action definition format 3 of KPM
 *
 * @details
 *
...
/*******************************************************************
 *
 * @brief Fill in the Indication Header format 1 of KPM
 *
 * @details
 *
...
/*******************************************************************
 *
 * @brief Fill in the matching conditional item for slice information of KPM
 *
 * @details
 *
 *    Function : kpmFillSliceMeasCondItem
 *
 *    Functionality: Fills MeasurementCondUEidItem_t of Indication Message
 *
...
/*******************************************************************
 *
 * @brief Deallocate the memory allocated for MeasurementInfoList_t 
 *
 * @details
 *
 *    Function : kpmFreeMeasInfoList
 *
 *    Functionality: freeing the memory allocated for MeasurementInfoList_t
 *
 * @param[in] measData, MeasurementInfoList_t*
 * @return ROK     - success
 *         RFAILED - failure
 *
 ******************************************************************/
...
/*******************************************************************
 *
 * @brief Deallocate the memory allocated for E2SM_KPM_IndicationMessage_Format1_t 
 *
 * @details
 *
 *    Function : kpmFreeMeasInfoList
 *
 *    Functionality: freeing the memory allocated for E2SM_KPM_IndicationMessage_Format1_t
 *
 * @param[in] measData, E2SM_KPM_IndicationMessage_Format1_t*
 * @return ROK     - success
 *         RFAILED - failure
 *
 ******************************************************************/
 ...
 *
 * @param[in] ricIndicaMsg, RICindicationMessage_t*
 * @param[in] format, int, indicate the Indication Message format, format 1 for cell metrics, format 2 for slice metrics
 * @return ROK     - success
 *         RFAILED - failure
 *
 ******************************************************************/
