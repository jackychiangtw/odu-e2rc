 ...
 * @details
 *
 *    Function : rcProcE2rcRanStructItem
 *
...
uint8_t rcProcE2rcRanStructItem(RANParameter_STRUCTURE_Item_t *ranStructItem){
    ...
    if(ranStructItem->ranParameter_valueType->present == RANParameter_ValueType_PR_ranP_Choice_ElementTrue){
        ...
        ret = rcProcE2rcRanElemFalse(ranStructItem->ranParameter_valueType->choice.ranP_Choice_ElementFalse, ranStructItem->ranParameter_ID);
        if(ret != ROK){
        ...
        ret = rcProcE2rcRanStruct(ranStructItem->ranParameter_valueType->choice.ranP_Choice_Structure);
        if(ret != ROK){
        ...
        ret = rcProcE2rcRanList(ranStructItem->ranParameter_valueType->choice.ranP_Choice_List);
        if(ret != ROK){
...
uint8_t rcProcE2rcRanStruct(RANParameter_ValueType_Choice_Structure_t *ranParaChoiceStruct){
    ...
    for(int i=0;i<ranParaStruct->sequence_of_ranParameters->list.count;i++){
        ret = rcProcE2rcRanStructItem(ranParaStruct->sequence_of_ranParameters->list.array[i]);
        if(ret != ROK){
...
uint8_t rcProcE2rcRanList(RANParameter_ValueType_Choice_List_t *ranParaChoiceList){
    ...
    for(int i=0;i<ranParaList->list_of_ranParameter.list.count;i++){
        ...
        for(int j=0;j<ranParaStruct->sequence_of_ranParameters->list.count;j++){
            ret = rcProcE2rcRanStructItem(ranParaStruct->sequence_of_ranParameters->list.array[j]);
            if(ret != ROK){
                DU_LOG("\nERROR  --> E2SM-RC: Failed to process list, %d",__LINE__);
                return RFAILED;
            }
 ...
 * @details
 *
 *    Function : rcProcE2rcRanElemFalse
 *
 ...
 * @details
 *
 *    Function : rcProcE2rcMessageFrmt1Item
 *
...
uint8_t rcProcE2rcMessageFrmt1Item(E2SM_RC_ControlMessage_Format1_Item_t *ctrlFrmt1){
    ...
    if(ctrlFrmt1->ranParameter_valueType.present == RANParameter_ValueType_PR_ranP_Choice_ElementTrue){
        ...
        ret = rcProcE2rcRanElemFalse(ctrlFrmt1->ranParameter_valueType.choice.ranP_Choice_ElementFalse, ctrlFrmt1->ranParameter_ID);
        if(ret != ROK){
        ...
        ret = rcProcE2rcRanStruct(ctrlFrmt1->ranParameter_valueType.choice.ranP_Choice_Structure);
        if(ret != ROK){
            DU_LOG("\nERROR  --> E2SM-RC: Failed to process structure, %d",__LINE__);
            return RFAILED;
        }
        ...
        ret = rcProcE2rcRanList(ctrlFrmt1->ranParameter_valueType.choice.ranP_Choice_List);
        if(ret != ROK){
            DU_LOG("\nERROR  --> E2SM-RC: Failed to process list, %d",__LINE__);
            return RFAILED;
        }
 ...
 * @details
 *
 *    Function : rcProcE2rcMessageFrmt1List
 *
...
uint8_t rcProcE2rcMessageFrmt1List(E2SM_RC_ControlMessage_Format1_t *ctrlFrmt1_list){
    ...
    for(int i=0;i<ctrlFrmt1_list->ranP_List.list.count;i++){
        ret = rcProcE2rcMessageFrmt1Item(ctrlFrmt1_list->ranP_List.list.array[i]);
        if(ret != ROK){
            DU_LOG("\nERROR  --> E2SM-RC: Failed to process list, %d",__LINE__);
            return RFAILED;
        }
 ...
 * @details
 *
 *    Function : rcProcE2rcCtrlMessage
 *
...
uint8_t rcProcE2rcCtrlMessage(RICcontrolMessage_t *ricControlMessage){
    ...
    if(ctrlMessage->ric_controlMessage_formats.present == E2SM_RC_ControlMessage__ric_controlMessage_formats_PR_controlMessage_Format2){
        ...
        policyIdx = 0;
        ret = rcProcE2rcMessageFrmt1List(ctrlMessage->ric_controlMessage_formats.choice.controlMessage_Format1);
        if(ret != ROK){
